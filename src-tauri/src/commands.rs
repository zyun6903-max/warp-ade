...
use crate::workspace::{checkout_branch, inspect_workspace, WorkspaceInfo};
...
pub fn create_project(
    ...
    state.db.create_project(&name, workspace_path.as_deref())
}
...
pub fn save_session_to_workspace(
    ...
    state.db.save_session_to_workspace(&session_id, &workspace_path)
}
...
pub fn save_project_to_workspace(
    ...
    state.db.save_project_to_workspace(&project_id, &workspace_path)
}
...
pub async fn test_provider(
    ...
    let provider = Provider {
        id: input.provider_id.unwrap_or_else(|| "test".to_string()),
        name: "test".to_string(),
        base_url: input.base_url,
        api_format: input.api_format,
        models: vec![input.default_model.clone()],
        default_model: input.default_model,
        priority: 1,
        enabled: true,
        has_key: true,
    ...
    Ok(test_provider_connection(&state.http, &provider, &api_key).await)
}
...
pub async fn send_message(
    ...
    if agent_mode.unwrap_or(settings.agent_enabled_default) {
